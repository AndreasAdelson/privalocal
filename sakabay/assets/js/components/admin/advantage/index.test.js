...
import AdvantageIndex from './index';
...
describe('Advantage entity index', () => {
  beforeEach(() => {
    ...
    wrapper = shallow(AdvantageIndex, {
      i18n,
      localVue,
    });
  });
...
describe('methods test', () => {
  it('refreshData() test', () => {
    wrapper.vm.filterFields = 'message';
    ...
    const expectedData = {
      params: {currentPage: 1, filter: 'testfilter', filterFields: 'message', perPage: 1, sortBy: 'still a test', sortDesc: false}
    };
    wrapper.vm.refreshData();
    expect(spyOnGet).toHaveBeenCalledWith('/api/admin/advantages',expectedData);
  });

  it('refreshData request success', async() => {
    axios.get.mockImplementationOnce(() => {
      return Promise.resolve({
        data: [{ id: 1, message: 'test1', code: 'RECH', priority: 1}],
        headers: { 'x-total-count': 4 }
      });
    });
