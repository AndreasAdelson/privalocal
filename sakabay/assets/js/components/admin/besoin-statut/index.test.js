...
import BesoinStatutIndex from './index';
...
describe('BesoinStatut entity index', () => {
  beforeEach(() => {
    ...
    wrapper = shallow(BesoinStatutIndex, {
      i18n,
      localVue,
    });
  });
...
describe('methods test', () => {
  it('refreshData() test', () => {
    ...
    expect(spyOnGet).toHaveBeenCalledWith('/api/admin/besoin-statuts',expectedData);
  });

  it('refreshData request success', async() => {
    axios.get.mockImplementationOnce(() => {
      return Promise.resolve({
        data: [{ id: 1, name: 'test1', code: 'VAL'}],
        headers: { 'x-total-count': 4 }
      });
    });
