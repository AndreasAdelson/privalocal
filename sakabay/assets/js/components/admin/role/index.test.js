...
describe('methods test', () => {
  it('refreshData() test', () => {
    ...
    expect(spyOnGet).toHaveBeenCalledWith('/api/admin/roles',expectedData);
  });
  ...
  it('refreshData request success', async() => {
    axios.get.mockImplementationOnce(() => {
      return Promise.resolve({
        data: [{
          ...
          name: 'test1',
          code: 'test code',
          fonctions: [{description: 'test fonction'}, {description: 'test fonction2'}]
        }],
        headers: { 'x-total-count': 4 }
      });
    });
    expect(await wrapper.vm.refreshData()).toEqual([{
      ...
      actions: 1,
      code: 'test code',
      fonctions: 'test fonction<br />test fonction2'
    }]);
