...
import SousCategoryShow from './index';
...
describe('Show a sousCategory entity', () => {
  beforeEach(() => {
    ...
    wrapper = shallow(SousCategoryShow, {
      i18n,
      localVue,
    });
  });
...
describe('created test', () => {
  it('creates with id', async() => {
    axios.get
    .mockImplementationOnce(() => Promise.resolve({data: {name: 'name test', category: {name: 'category name'}}}));
    wrapper.vm.sousCategoryId = 10;
    wrapper.vm.loading = true;
    await SousCategoryShow.created.bind(wrapper.vm)();
    expect(wrapper.vm.sousCategory).toEqual({name: 'name test', category: {name: 'category name'}});
