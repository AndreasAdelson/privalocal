...
describe('Create or edit a company entity', () => {
  beforeEach(() => {
    ...
    wrapper = shallow(SubscriptionForm, {
...
describe('created test', () => {
  it('creates with id', async() => {
    ...
    .mockImplementationOnce(() => Promise.resolve({data: [
      {message: 'advantage message test'}
    ]}))
    .mockImplementationOnce(() => Promise.resolve({data: {
      name: 'name test',
      code: 'TEST',
    ...
    await SubscriptionForm.created.bind(wrapper.vm)();
    expect(wrapper.vm.formFields.name).toEqual('name test');
    expect(wrapper.vm.loading).toEqual(false);
    expect(wrapper.vm.advantages).toEqual([{message: 'advantage message test'}]);
  });
