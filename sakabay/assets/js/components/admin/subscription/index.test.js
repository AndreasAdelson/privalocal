...
import SubscriptionIndex from './index';
...
describe('Subscription entity index', () => {
  beforeEach(() => {
...
describe('methods test', () => {
  it('refreshData() test', () => {
    ...
    const expectedData = {
      params: {currentPage: 1, filter: 'testfilter', filterFields: 'name', perPage: 1, sortBy: 'still a test', sortDesc: false}
    };
    wrapper.vm.refreshData();
    expect(spyOnGet).toHaveBeenCalledWith('/api/admin/subscriptions',expectedData);
  });
  ...
  it('refreshData request success', async() => {
    axios.get.mockImplementationOnce(() => {
      return Promise.resolve({
        data: [{
          ...
          code: 'test code',
          price: 'test price',
          advantages: [{message: 'message advanate test'}, {message: 'message advanate test2'}]
        }],
        headers: { 'x-total-count': 4 }
      });
    });
    expect(await wrapper.vm.refreshData()).toEqual([{
      ...
      code: 'test code',
      price: 'test price',
      advantages: 'message advanate test<br />message advanate test2'
    }]);
